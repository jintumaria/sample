...
try
{
	String itemName="",itemQty="",itemRate="",quoteNo="";
	...
	{
		quoteNo = request.getParameter("value3");
	}	
	
	
	String idq="";
			String query_idq="Select id from quote_tab where ino='"+quoteNo+"'";
			...
			int COUNT_I=0;
	        String query="Select * from add_cart where userId='"+session.getAttribute("admin_id").toString()+"' and sessionId='"+quoteNo+"' and itemName='"+itemName+"' ";
			...
			if(COUNT_I==0)
			{
					...
					String sql="insert into add_cart(userId,sessionId,itemName,addQty,addRate,status,createdDate,modifiedDateTime)values('"+session.getAttribute("admin_id").toString()+"','"+quoteNo+"','"+itemName+"','"+itemQty+"','"+itemRate+"','A',now(),now())";			
            ...
            <table id="example" class="table table-bordered table-striped" style="width: 100%; cellspacing: 0;">
						...
						int j=0;
						 double value_amount=0.00,tot_value_amount=0.00;
			  			String query_invoice_temp="select * from add_cart where userId='"+session.getAttribute("admin_id")+"' and sessionId='"+quoteNo+"' and status='A' order by modifiedDateTime desc";
